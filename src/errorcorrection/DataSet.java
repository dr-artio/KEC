...
//					System.out.println(name);
    ...
    public DataSet(DataSet ds, String idmethod) // <editor-fold defaultstate="collapsed" desc=" DESCRIPTION ">
	{
		freqThr = ds.freqThr;
		lengthThr = ds.lengthThr;
		additionalfreqThrMult = ds.additionalfreqThrMult;
		...
		k = ds.k;
                ...
                if (idmethod.equalsIgnoreCase("ET"))
                {
                    StringTokenizer st = new StringTokenizer(ds.reads.get(0).name,"_");
                    tag = st.nextToken() + "_" + st.nextToken();
                }
...
//				km.setKCount1(kCount1);
    ...
    void clustering() throws IOException {
        ...
        String pref = f1.getParent() + File.separator;
        int count = 0;
        int count1 = 0;
        for (Read r : reads) {
            count++;
...
//				 fw.write(km.StartPos+ " " + km.getCount() + "\n");
            ...
            String[] exPath = new String[] {"fams" + File.separator +
                    "fams", "0", "0", "200", 
                    "kmer" + count + "data", pref};
            p = run.exec(exPath, null, ErrorCorrection.env_path);
//			 p=run.exec("fams 0 0 200 " + "kmer" + count + "data .\\"); 
            ...
            if (p.exitValue() != 0) {
                System.err.println("Error in clustering program");
                continue;
            }

            File fout = new File(pref + "out_" + "kmer" + count + "data.txt");
...
//			 System.out.println("Deleting files: read " + count + "/" + reads.size()+"\n");	
            File f = new File(pref + "modes_kmer" + count + "data.txt");
            f.deleteOnExit();
            f = new File(pref + "pilot_200_kmer" + count + "data.txt");
            f.deleteOnExit();
            f = new File(pref + "out_kmer" + count + "data.txt");
            f.delete();
            int i;
            f.deleteOnExit();
...
//			r.errorRegions = new ArrayList<ErrorRegion>();
            ...
            while (i <= r.getLength() - k) {
                if (a[i] <= freqThr) {
                    ...
                    if (l > 0) {
                        if ((Math.abs(b[i] - clustprm) < 0.1) && 
                                (a[i] <= freqThr * additionalfreqThrMult) && 
                                (clustprm >= 0)) {
                            l++;
                            s += r.getNucl(i);
                            r.NuclType[i] = 1;
                            ...
                            while (upd_start >= 0) {
                                if ((Math.abs(b[upd_start] - clustprm) < 0.1) &&
                                        (a[upd_start] <= freqThr * additionalfreqThrMult) && 
                                        (clustprm >= 0)) {
                                    l++;
...
//                        int len = Math.min(e-b+1, homoplen);
    ...
    void PrintReadsSeparateFilesKeepName(String addr) throws FileNotFoundException, IOException // <editor-fold defaultstate="collapsed" desc=" DESCRIPTION ">
    {
        FileReader fr = new FileReader(addr);
        BufferedReader br = new BufferedReader(fr);
        int i = 1;
...
// </editor-fold>
    ...
    Read findMostFreqRead() // <editor-fold defaultstate="collapsed" desc=" DESCRIPTION ">
    {
        Read maxr = new Read();
        double maxfreq = 0;
    ...
    void printpairdistReads(String addr) throws IOException // <editor-fold defaultstate="collapsed" desc=" DESCRIPTION ">
    {
    ...
    // </editor-fold>
    public ArrayList<Read> FindMostFreqHaplotypes(int n) throws IOException
	{
		HaplComp hc = new HaplComp();
		Collections.sort(haplotypes, hc);
		
                ArrayList<Read> ar = new ArrayList<Read>();
		for (int  i = 0; i < n; i++)
		{
                    Haplotype h = haplotypes.get(i);
			ar.add(new Read(h.nucl,h.frequency));
		}
                return ar;
	}
}
