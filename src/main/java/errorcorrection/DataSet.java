...
/*                            Corrector cr = new Corrector();
        ...
        public DataSet(DataSet ds, String idmethod)
	{
		freqThr = ds.freqThr;
		lengthThr = ds.lengthThr;
		additionalfreqThrMult = ds.additionalfreqThrMult;
...
/*		int i = 0;
	...
	public void PrintHaplotypes(String outfile) throws IOException
	{
        ...
        public void PrintHaplotypes(String outfile, String idmethod) throws IOException
	{
		FileWriter fw = new FileWriter(outfile);
		HaplComp hc = new HaplComp();
		Collections.sort(haplotypes, hc);
		...
		for (Haplotype h : haplotypes)
		{
			int decimalPlace = 4;
                        ...
                        {
                            fw.write(">" + h.name + "_" + h.concentration + "\n"+ h.nucl + "\n");
                        }
		}
		fw.close();
	}
	...
	public void findHaplotypes() throws IOException
	{
		...
		for (Haplotype h : haplotypes)
			h.concentration = h.concentration / allconc;
        ...
        public void findHaplotypes(String idmethod) throws IOException
	{
		haplotypes = new ArrayList<Haplotype>();
		StrComp sc = new StrComp();
		Collections.sort(reads, sc);
        ...
        public void findHaplotypes(String idmethod, String idsort) throws IOException
	{
		haplotypes = new ArrayList<Haplotype>();
...
// </editor-fold>
        ...
        Haplotype findMostFreqHaplotype()
                // <editor-fold defaultstate="collapsed" desc=" DESCRIPTION ">
        {
            Haplotype maxh = this.haplotypes.get(0);
            double maxconc = 0;
            for (Haplotype h : this.haplotypes)
                if (h.concentration > maxconc)
                {
                    maxconc = h.concentration;
                    maxh = h;
                }
            return maxh;
        }
        ...
        public ArrayList<Read> FindMostFreqHaplotypes(int n) throws IOException
	{
        ...
        public ArrayList<Read> FindMostFreqReads(int n) throws IOException
	{
		ReadComp rc = new ReadComp();
		Collections.sort(reads, rc);
        ...
        public boolean containRead(Read s)
        {
            for (Read r : reads)
